...
# 성능 참고
## 실시간 검색어 / 열람 기록
* 실시간 검색어는 `viewlog` 원본 로그가 아니라 `pageview_daily` 일별 집계 테이블을 사용합니다.
* 익명 방문은 실시간 검색어 집계에만 반영되고, 개인 열람 기록용 `viewlog`에는 저장되지 않습니다.
* 로그인 사용자의 열람 기록만 `viewlog`에 남으며, 서버는 매일 오래된 `viewlog`와 `pageview_daily` 데이터를 정리합니다.
