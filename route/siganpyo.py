...
async def siganpyo(grade = '', class_ = '', ymd = ''):
                ...
                if student_id_row and student_id_row[0]:
                    ...
                    try:
                        if len(student_id) >= 2 and student_id.isdigit():
                            grade = student_id[0]
                            class_ = student_id[1]
                            if not (1 <= int(grade) <= 3 and 1 <= int(class_) <= 7):
                                grade = '1'
                                class_ = '1'
                        else:
                            grade = '1'
        ...
        if grade != '' and class_ != '':
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    content = f"{ymd[0:4]}년 {ymd[4:6]}월 {ymd[6:8]}일 {grade}학년 {class_}반 시간표<br><br>"
        ...
        form_html = f'''
            <form onsubmit="window.location.href = '/siganpyo/' + document.getElementById('grade').value + '/' + document.getElementById('class_nm').value + '/' + document.getElementById('date_picker').value.replace(/-/g, ''); return false;">
                ...
                <select id="class_nm" name="class_nm">
                    {"".join([f'<option value="{i}" {"selected" if class_ == str(i) else ""}>{i}반</option>' for i in range(1, 8)])}
                </select>
